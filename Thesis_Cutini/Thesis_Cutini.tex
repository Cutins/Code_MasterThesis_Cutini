...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
where $d_{\text{des}}$ is the reference distance between kart and runner.
Using this reference, the infinite horizon optimal control problem \ref{eq:LQR} is solved to obtain, in closed form, the optimal unconstrained solution $u^*$.
...
\begin{figure}
	\centering
	\includegraphics[width=1.0\textwidth]{LQR_scheme.png}
	\caption{Linear Quadratic Regulator control scheme}
	\label{image:LQR_scheme}
\end{figure}
...
\label{alg:LQR_implementation}
\end{algorithmic}
\end{algorithm}

The overall control scheme is the one shown in \ref{image:LQR_scheme}, while algorithm \ref{alg:LQR_implementation} describes the points followed by the LQR algorithm while running real-time on the system.
...
The term relative to the actual runner acceleration can be seen as the introduction of a known disturbance in the system.
...
The optimal input sequence, found by repartively solving at each time step the optimizaton problem, is such that $\boldsymbol{u}^* \in \mathbb{R}^{m \times N}$.
...
\begin{figure}
	\centering
	\includegraphics[width=1.0\textwidth]{MPC_scheme.png}
	\caption{Model Predictive Control scheme}
	\label{image:MPC_scheme}
\end{figure}
...
\label{alg:MPC_implementation}
\end{algorithmic}
\end{algorithm}

The overall MPC control scheme is shown in \ref{image:MPC_scheme}.
It is easy to understand the semplification in the architecture achieved with the MPC, with respect to the LQR case shown in \ref{image:LQR_scheme}.
 Algorithm \ref{alg:MPC_implementation} describes the points of the MPC real-time implementation.
...
\subserction{State estimator e dati dei sensori usati}
Mostrare che in LQR
x error è uguale a h che esce dal lidar.
